...
function usage() {
  console.log(`agent-context-diff v${VERSION}

Usage:
  agent-context-diff --path FILE_OR_DIR
  agent-context-diff FILE_OR_DIR --markdown
  agent-context-diff FILE_OR_DIR --sarif > results.sarif
  agent-context-diff FILE_OR_DIR --annotations
  agent-context-diff FILE_OR_DIR --redact
...
try {
  ...
  console.error(`agent-context-diff: ${error.message}`);
  process.exit(2);
}
