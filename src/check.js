...
export const CHECKS = [
  ...
  [
    "removed",
    "removed|deleted|删除|移除",
    "Lists removed context."
  ],
  [
    "risk",
    "risk|impact|风险|影响",
    "Mentions risk or impact."
  ],
  [
    "verify",
    "verify|test|review|验证|审查",
...
export function checkText(text, target = 'input') {
  ...
  return { tool: 'agent-context-diff', target, score, passed, total: results.length, results, redacted: source };
}
...
export function formatText(report) {
  const lines = [`Agent Context Diff score: ${report.score}/100`];
...
export function formatMarkdown(report) {
  ...
  return `# Agent Context Diff Report\n\nScore: **${report.score}/100**\n\n| Check | Status | Message |\n| --- | --- | --- |\n${rows}\n`;
}

export function formatAnnotations(report) {
  return report.results.filter((item) => !item.passed).map((item) => `::warning title=Agent Context Diff ${item.id}::${item.message}`).join('\n');
}
...
export function formatSarif(report) {
  return {
    version: '2.1.0',
    runs: [{
      tool: { driver: { name: 'agent-context-diff', informationUri: 'https://github.com/aolingge/agent-context-diff', rules: report.results.map((item) => ({ id: item.id, shortDescription: { text: item.message } })) } },
